...
const getCardByIdOrFirst = async (req: Request, res: Response, next: NextFunction) => {
  const { cardId, nextId, grade, stack } = req.body;
      ...
      if (cardId && nextId && grade) {
        ...
        switch (grade) {
          case 1: {
            ...
            const filteredCardArray: Card[] = allCards.reduce((acc: Card[], obj: Card) => {
              if (obj.stack[0] === currentCard.stack[0] && obj.stack[1] === currentCard.stack[1]) {
                acc.push(obj);
              }
              return acc;
            }, []);

            let lastCard = filteredCardArray.find(obj => !obj.next);
            if (lastCard?._id.equals(currentCard._id) || !lastCard) {
              return res.status(406).json({
                message: `Illegal recursion due to self-reference.`,
              });
            }
            let updateObject: { next: ObjectId };
            updateObject = { next: currentCard!._id };
            await updateCardById(lastCard._id, updateObject);

            await unsetNextFieldOnCardById(currentCard._id);
            break;
          }
          case 2: {
            const sortedCards = await retrieveCardsSortedByReference(session_id[0], stack);

            if (!sortedCards || sortedCards.length < 2) break;
            // console.log(sortedCards.length)

            const insertAt = Math.floor(sortedCards.length - (1/4 * sortedCards.length));
            
            sortedCards.shift();

            let updateObject: { next: ObjectId };
            updateObject = { next: currentCard!._id };
            await updateCardById(sortedCards[insertAt - 1]._id, updateObject);
            
            updateObject = { next: sortedCards[insertAt]._id };
            await updateCardById(currentCard._id, updateObject);
          ...
          case 3: {
            ...
            const insertAt = Math.floor(sortedCards.length - (2/4 * sortedCards.length));
          ...
          case 4: {
            ...
            const insertAt = Math.floor(sortedCards.length - (3/4 * sortedCards.length));
          ...
          default: {
            return res.status(406).json({
              message: `Illegal grade provided.`,
            });
          }
        }
  ...
  try {
    ...
    if (isSessionValid) {
      ...
      allCards?.forEach(async selectedCard => {
        if (selectedCard.next == new ObjectId(cardId)) {
          await unsetNextFieldOnCardById(selectedCard._id);
        }
      });
